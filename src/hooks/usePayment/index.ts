...
import { queryPayments, paymentsAtom, startPageAtom } from '../../atoms/payment';
import { getPayments } from '../../services/payment';
import Plan from '../../entities/Plan';

const usePayment = () => {
  const [ isLoading, setIsLoading ] = useState(false);
  const [ query, setQuery ] = useAtom(queryPayments);
  ...
  const filtered = useMemo(() => payments.filter(({ firstName, lastName }) =>
    query !== "" ? ( firstName.includes(query) || firstName.includes(lastName)) : true
  ), [payments, query]);
