...
export const Wrapper = styled.div<IWrapperProps | any>`
  width: 100%;
  height: 100%;
  ...
  background: ${({ active }) => active === 'true' && `${Colors.colorTextLinkActive}`};
  ...
  span {
    ...
    align-items: center;
    text-align: center;
    color: ${({ active }) => active === 'true' ? `${Colors.white}`: `${Colors.backgroundColorButton}`};
  }
