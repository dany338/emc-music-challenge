...
export interface ILoadMoreProps {
  title?: string;
  startPage?: number;
  onClick: ((page: number, limit: number, activeLoadMore: boolean) => void) | any;
  type?: string;
};

const LoadMore: React.FC<ILoadMoreProps> = ({ title = 'Load more', startPage = 0, onClick, type = 'load' }) => (
  <Container data-testid="loadmore-container">
    <Wrapper onClick={() => type !== 'tab' ? onClick(startPage + 20, 20, true) : onClick() }>
      <span>{title}</span>
