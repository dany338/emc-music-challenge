...
const TextField: React.FC<ITextFieldProps> = ({ type, typeInput, name, placeholder, value, onChange, errors = null, icon = null, rows = '4', position = 'center' }) => {
  const [changeType, setChangeType] = useState(type);
  const handleChangeType = () => setChangeType(changeType === 'password' ? 'text' : 'password');
  return (
    <Container>
      <Wrapper>
        <SearchBox position={position} type={changeType}>
          {typeInput === 'textarea' ? (
